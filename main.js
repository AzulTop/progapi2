...
setTimeout(function(){let emailval=document.getElementById("email").value
name=document.getElementById("name").value,phone=document.getElementById("phoneno").value,op1=document.getElementById("chosehtml").checked,op2=document.getElementById("choseplain").checked,chose50cc=document.getElementById("S50cc").checked,chose250cc=document.getElementById("S250cc").checked,chose500cc=document.getElementById("S500cc").checked,chose600cc=document.getElementById("S600cc").checked,chose601cc=document.getElementById("S601cc").checked,Chosestandard=document.getElementById("Standard").checked,Chosecruiser=document.getElementById("Cruiser").checked,Chosesport=document.getElementById("Sport").checked,ChosesTouring=document.getElementById("Touring").checked,ChosesScooter=document.getElementById("Scooter").checked,ChoseOffroad=document.getElementById("Offroad").checked
...
localStorage.setItem("Offroad",ChoseOffroad)},6000);function load(){const storedEmail=localStorage.getItem("email")
...
if(storedEmail,storedName,storedNo,styleop,styleop2,Engchoice,Engchoice2,Engchoice3,Engchoice4,Engchoice5,StandardT,CruiserT,SportT,TouringT,ScooterT,OffroadT){document.getElementById("email").value=storedEmail
...
function cleardata(){window.localStorage.clear()}
const myLatLng={lat:52.9029218,lng:-1.8870629};const myLatLng2={lat:52.9866342,lng:-2.2034458};const myLatLng3={lat:53.0807631,lng:-2.4435213};function initMap(){const defaultpos={lat:53.0002046,lng:-2.1587255};let map=new google.maps.Map(document.getElementById("map"),{zoom:10,center:defaultpos});const marker=new google.maps.Marker({position:myLatLng,map:map,title:"Uttoxeter"});const marker2=new google.maps.Marker({position:myLatLng2,map:map,title:"Stoke-on-Trent"});const marker3=new google.maps.Marker({position:myLatLng3,map:map,title:"Crewe"})}
function GetPoS(){let map=new google.maps.Map(document.getElementById('map'),{center:{lat:52.9046891,lng:-2.1505637},zoom:6});const infoWindow=new google.maps.InfoWindow({map:map});if(navigator.geolocation){navigator.geolocation.getCurrentPosition(function(position){let pos={lat:position.coords.latitude,lng:position.coords.longitude};infoWindow.setPosition(pos);infoWindow.setContent("Location found.");map.setCenter(pos)},function(){handleLocationError(!0,infoWindow,map.getCenter())})}else{handleLocationError(!1,infoWindow,map.getCenter())}}
...
function CloserMark(){}
function Showmenu(){if(Mmenu.style.display=="none")<!--If element isn't visible by default when clicked will be visible-->
